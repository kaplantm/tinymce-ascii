...
function App() {
  ...
  // const tinyEditorViewRef = useRef(null);
  const [tinyEditorContent, setTinyEditorContent] = useState("");
  // const { ascii } = useAsciiImage(frame, canvasRef);

  const images = tinyEditorContent.match(/<img src=".*"/g);
  const imageUrls = images?.map((image) => ((image.match(/".*"/g) || [])[0]?.split('"') || [])[1]) || [];
  const frames = useGifFrames(imageUrls[0]);

  console.log({ frames });

  const onEditorChange = (value: string, editor: Editor) => {
    setTinyEditorContent(value);
  };

  return (
    <div>
      <canvas ref={canvasRef} style={{ display: "none" }} />
      {/* {ascii ? <TinyEditorView copy={ascii} ref={tinyEditorViewRef} /> : "Loading..."} */}
